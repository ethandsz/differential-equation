...
def pde_residual(x, y, z, t, model, sourceRegion):
    ...
    pde_problem = PollutionPDE(
            num_cells=25,
            diffusion_coef=model.delta.item(),
...
# 2. Access the components
    ...
    predicted_var = CellVariable(name="pred_poll", mesh=mesh, value=c.cpu().detach().numpy())
    residual = eq.sweep(var=predicted_var, dt=1/60)
